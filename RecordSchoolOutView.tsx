...
export function RecordSchoolOutView() {
  ...
  const [records, setRecords] = useLocalStorage<AttendanceRecord[]>('school-out-records', []);
  ...
  const handleRecord = async () => {
    ...
    if (alreadyRecorded) {
      alert('Pelajar ini telah direkodkan keluar ke sekolah hari ini.');
      return;
    }
    ...
    const newRecord: AttendanceRecord = {
      id: `KHD${String(records.length + 1).padStart(3, '0')}`,
  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-end">
        <div>
          <h1 className="text-2xl font-bold text-slate-800">Kehadiran Ke Sekolah (Keluar Asrama)</h1>
          <p className="text-sm text-slate-500 mt-1">Imbas QR pelajar untuk merekod pergerakan keluar ke sekolah.</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 max-w-4xl">
        ...
        <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-6 flex flex-col">
          <div className="mb-6 bg-indigo-50 p-4 rounded-lg border border-indigo-100">
            <label className="block text-sm font-medium text-indigo-900 mb-2">Tarikh Kehadiran Direkodkan</label>
            <input 
              type="date" 
              value={recordDate} 
              onChange={(e) => setRecordDate(e.target.value)} 
              className="w-full text-sm border-indigo-200 rounded-lg shadow-sm focus:border-indigo-500 focus:ring-indigo-500 p-2 border bg-white" 
            />
          </div>
          ...
          {method === 'qr' ? (
            <div className="flex flex-col items-center justify-center p-8 border-2 border-dashed border-slate-300 rounded-xl bg-slate-50">
              ...
              <p className="text-sm text-slate-500 text-center">Halakan kamera pada Kod QR pas pelajar untuk merekod kehadiran.</p>
            ...
            <form className="space-y-4" onSubmit={handleSearch}>
              <div>
                ...
                <div className="flex space-x-2">
                  ...
                  />
                  <button type="submit" className="bg-indigo-600 hover:bg-indigo-700 text-white px-4 py-2.5 rounded-lg transition-colors font-medium">Cari</button>
                </div>
              </div>
            </form>
          )}
            ...
            {scannedStudent ? (
              <>
                <div className="space-y-4">
                  <div className="flex items-start space-x-4">
                    ...
                    <div>
                      ...
                      <div className="inline-flex items-center mt-2 px-2.5 py-0.5 rounded-full text-xs font-medium bg-indigo-100 text-indigo-700 border border-indigo-200">
                        <Icons.BookOpen className="w-3 h-3 mr-1" /> Bersedia Ke Sekolah
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="bg-slate-50 rounded-lg p-4 border border-slate-200 text-sm">
                    <div className="grid grid-cols-2 gap-y-2">
                      ...
                      <div className="text-slate-500">Sesi:</div>
                      <div className="font-medium text-slate-800">Pagi</div>
                    </div>
                  </div>
                </div>
                ...
                <button 
                  ...
                  className="w-full mt-6 bg-indigo-600 hover:bg-indigo-700 text-white py-3 rounded-lg font-bold text-lg shadow-sm transition-colors flex items-center justify-center"
                >
                  <Icons.ArrowUpRight className="w-5 h-5 mr-2" /> REKOD KELUAR KE SEKOLAH
                </button>
              </>
            ) : (
      ...
      <div className="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden mt-8">
        <div className="p-4 border-b border-slate-200 bg-slate-50 flex justify-between items-center">
          <h2 className="text-lg font-semibold text-slate-800 flex items-center">
            <Icons.ListChecks className="w-5 h-5 mr-2 text-slate-500" /> Log Kehadiran Ke Sekolah
          </h2>
          <div>
            <input 
              type="date"
              ...
              className="text-sm border-slate-300 rounded-lg shadow-sm focus:border-indigo-500 focus:ring-indigo-500 p-2 border"
            />
          </div>
        </div>
          ...
          <table className="w-full text-left text-sm text-slate-600">
            <thead className="bg-slate-50 text-slate-500 font-medium border-b border-slate-200">
              <tr>
                ...
                <th className="py-3 px-4 text-center">Masa Keluar</th>
            ...
            <tbody className="divide-y divide-slate-200">
              {records.filter(r => r.date === filterDate).map((record, i) => (
                <tr key={i} className="hover:bg-slate-50 transition-colors">
                  ...
                  <td className="py-3 px-4 font-semibold text-indigo-600">{record.studentId}</td>
                  ...
                  <td className="py-3 px-4 text-center">{record.timeOut}</td>
              ...
              {records.filter(r => r.date === filterDate).length === 0 && (
                <tr>
                  <td colSpan={7} className="py-8 text-center text-slate-500">
                    Tiada rekod kehadiran pada {new Date(filterDate).toLocaleDateString('ms-MY', { day: 'numeric', month: 'long', year: 'numeric' })}.
                  </td>
                </tr>
              )}
            </tbody>
